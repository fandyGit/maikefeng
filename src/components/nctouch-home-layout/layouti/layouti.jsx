        ...
        >
          {
            this.props.layouti.map((item,index)=>{
              return (
                <a href={`http://www.51mkf.com/wap/tmpl/product_detail.html?goods_id=${item.goods_id}`} key={index}>
                  <img className='pic' src={item.goods_image} alt=""/>
                  <i className='shengyuehao'  style={{margin:'10px',fontSize:'12px',color:'#333'}} >{item.goods_name}</i>
